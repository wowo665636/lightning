...
public class StatisticsAVTopo {
	public static void main(String[] args) {
		...
		/*SpoutConfig kafkaConfig = new SpoutConfig(new ZkHosts(SPOUT_ZK_HOST,KAFKA_BROKER_ZK_PATH),
				"xpstrackingav", "/kafka-0.10.2.1/consumers", "bpd-statistics-av-1c");///kafka-0.10.2.1-test
		...
		builder.setBolt("statistics-bolt",new StatisticsAVBolt(),9).shuffleGrouping("statistics-spout").setNumTasks(9);
		...
		try {
			...
			StormSubmitter.submitTopologyWithProgressBar("bpd-statistics-av",config , builder.createTopology());
		} catch (Exception e) {
			e.printStackTrace();
		}*/
