 ...
 */
@Deprecated
public class StatisticsViewBolt extends AbstractTrackLog implements IRichBolt {
    ...
    public void execute(Tuple tuple) {
        try{
            ...
            String repeat_key = sb.append("V_").append(id).append("_").append(timestamp).toString();
            ...
            /**2.统计加载次数**/
            jedisCluster.hincrBy(Constant.BI_SSTICS_V_ALL_+day,hour,1);
            jedisCluster.expire(Constant.BI_SSTICS_V_ALL_+day,EXPIRE_TIME);
            // 统计 空广告
            /*if(checkEmptyAdvertising(trackLog)){
                jedisCluster.hincrBy(Constant.BI_SSTICS_V_EMPTY_+day,hour,1);
                jedisCluster.expire(Constant.BI_SSTICS_V_EMPTY_+day,EXPIRE_TIME);
                collector.ack(tuple);
                return ;
            }*/
            /**过滤区分品牌错误数据**/
            ...
            if("1".equals(trackLog.getAdgres())){
                //品牌点击量
                jedisCluster.hincrBy(Constant.BI_SSTICS_V_+day,Constant.BRAND_COUNT_+hour,1);
                jedisCluster.expire(Constant.BI_SSTICS_V_+day,EXPIRE_TIME);

            }
            if("2".equals(trackLog.getAdgres())||"4".equals(trackLog.getAdgres())){
                /**中长尾点击量**/
                jedisCluster.hincrBy(Constant.BI_SSTICS_V_+day,Constant.SURPLUS_COUNT_+hour,1);
                jedisCluster.expire(Constant.BI_SSTICS_V_+day,EXPIRE_TIME);

            }

            /**4.计算收入  cpm**/
            if(checkIncome(trackLog)&&"2".equals(trackLog.getBidtype())){
                if("1".equals(trackLog.getAdgres())){
                    // 品牌
                    jedisCluster.hincrBy(Constant.BI_SSTICS_BRAND_CONSUME_+day,Constant.V_BRAND_+hour,Long.valueOf(trackLog.getCharge()));
                    jedisCluster.expire(Constant.BI_SSTICS_BRAND_CONSUME_+day,EXPIRE_TIME);
                    //cpc 统计品牌收入计算次数
                    jedisCluster.hincrBy(Constant.BI_SSTICS_CPM_+day,Constant.V_BRAND_+hour,1);
                    jedisCluster.expire(Constant.BI_SSTICS_CPM_+day,EXPIRE_TIME);
                }

                if("2".equals(trackLog.getAdgres())||"4".equals(trackLog.getAdgres())){
                    jedisCluster.hincrBy(Constant.BI_SSTICS_SURPLUS_CONSUME_+day,Constant.V_SURPLUS_+hour,Long.valueOf(trackLog.getCharge()));
                    jedisCluster.expire(Constant.BI_SSTICS_SURPLUS_CONSUME_+day,EXPIRE_TIME);
                    //cpc 统计中长尾收入计算次数
                    jedisCluster.hincrBy(Constant.BI_SSTICS_CPM_+day,Constant.V_SURPLUS_+hour,1);
                    jedisCluster.expire(Constant.BI_SSTICS_CPM_+day,EXPIRE_TIME);
                }
