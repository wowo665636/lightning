 ...
 */
public class SplitAVBolt implements IRichBolt {
    private static final long serialVersionUID = -2256076439571548575L;
    //private static Logger log = LoggerFactory.getLogger(SplitBolt.class);

    private OutputCollector collector;

    private static final Pattern p_v= Pattern.compile("^id=(.*?)\t");// 1.新日志  id 2.isspam 反作弊
    ...
    public void execute(Tuple tuple) {
        ...
        if(checkUtil.checkRepeat(type_vid,vid_value,time)){
            ...
            checkUtil.setRepeatkey(Constant.TC_REPEAT_AV_KEY+tt3_hour_value,vid_value,EXPIRE_TIME);
            collector.ack(tuple);
            return ;
        }
        ...
        try{
            ...
            if(bidtype.trim().equals("3")){
                ...
                //checkUtil.setBackLog(impkey+"_"+hour_field_key,type_vid+"_"+vid_value+"_"+time,log_value,EXPIRE_TIME);
